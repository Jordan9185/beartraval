...
export type InboxInput = { title: string | null; rawText: string; imageBase64: string[] };
export type ValidatedResult = Omit<z.infer<typeof InboxResult>, "items"> & {
  items: Array<z.infer<typeof Item> & { auto_archive: boolean }>;
};

const SYSTEM_PROMPT = `你是 BeaRTravel 分享內容整理器。只辨識使用者實際分享的文字與圖片，不開啟網址、不推斷網址內的貼文或影片內容。
輸出所有明確出現的地點、商品，以及來源明示的行程天數和順序。每項 source_span 必須是輸入文字的原文短句；若只來自第 1 張圖片，寫 image:1（依序類推）。
沒有店名、商品名稱或停靠點時，items 和 template_days 留空。「不要去／避雷／不要買」提到的對象不是收藏或行程停靠點。地點有多個分店而來源未指明時，confidence 用 medium 或 low。
...
function anchored(span: string, input: InboxInput): boolean {
  ...
  return input.rawText.includes(span) || (input.title ?? "").includes(span);
}
...
function negativeContext(span: string, input: InboxInput): boolean {
  ...
  const text = input.rawText.includes(span) ? input.rawText : input.title ?? "";
...
export function validateResult(input: InboxInput, raw: unknown): ValidatedResult {
  ...
  const items = parsed.items.filter((item) => anchored(item.source_span, input)).map((item) => {
    ...
    const textEvidence = !source.startsWith("image:") && source.includes(name);
    const ambiguous = /(?:或|可能|附近|哪家|分店不明)/.test(item.source_span);
    return {
      ...item,
      auto_archive: item.origin_type === "explicit" && item.confidence === "high" && textEvidence &&
...
export async function organizeCapture(client: Anthropic, input: InboxInput, model = "claude-sonnet-5"):
  ...
  content.push({ type: "text", text: JSON.stringify({ title: input.title, text: input.rawText.slice(0, 20000) }) });
