...
| `src/validate.ts` | 對照原文檢查草稿：日期超出旅程、時間格式錯誤、原文片段對不上、沒有地點名稱 → 降級為待確認 |
| `eval/cases.ts` | 評測集（22 份，**目前全為合成資料**） |
...
npm run eval:dry    # 用預期答案自我檢查評分器（不呼叫 API）
npm run eval        # 實際呼叫 Claude API，需要 ANTHROPIC_API_KEY，會產生費用
npm run eval -- --effort medium --only seoul-korean,hiroshima-japanese
```
...
- 首爾 15 份、廣島 6 份、無地點 1 份；來源格式涵蓋 ChatGPT markdown、LINE 對話、備忘錄、英日韓中混合。
