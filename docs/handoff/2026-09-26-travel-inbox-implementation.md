...
- 收藏詳情原本能看見 `address_hint`，但「給計程車司機看」只讀 Place 地址，未定位地點會顯示沒有地址。現在卡片會沿用收藏地址線索；有韓文地址時優先於 Apple 地圖回傳的中文地址。線索仍標示需要核對分店與地圖位置，不當成已確認座標。
- 本機 Swift 套件 160 項通過，另重跑司機卡 11 項通過；Release 簽章建置成功，已覆蓋安裝至 BeaR 與 Jordan 的 iPhone。尚無解鎖後的卡片畫面人工驗收。
- 使用者指出收藏後仍找不到排進旅程的方式。現況：已定位收藏需從「試算順路」進入；未定位收藏只能「補填地點」，即使有韓文地址也無法選日期。已重寫[辨識到出發完整使用流程與開發計畫 v2](../planning/recognition-to-trip-journey-v2.md)：優先串起辨識、收藏／購物、排入某天、地圖與司機卡。專用的「排進旅程」流程及未定位 Stop 尚未實作，不能視為已交付。
