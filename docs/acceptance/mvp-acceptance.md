...
對應 issue #15、規劃 §7。日期：2026-09-24。
...
| 代號 | 內容 | 執行方式 |
|---|---|---|
| SQL | `supabase/tests/*.test.sql`：9 組、159 項斷言，另含 2 個兩連線同時寫入的測試 | `supabase/tests/run.sh`（CI：DB tests） |
| UT | `Packages/AppCore` 單元測試 89 項 | `swift test --package-path Packages/AppCore`（CI：iOS） |
...
| AI | `ai/itinerary-parse`、`ai/trip-assistant` 單元測試與 eval dry-run | CI：AI tests；真實 eval 需 API key |
...
| AC-02 | 確認分店與 Fixed 訂位後建立 Trip | IT `importKeepsTextThroughFailedParseAndCommits`、`snapshotMatchesServerRevision`；Today／Trip／Map 共用 `TripSnapshot` 並顯示資料版本 r#；Base Route 綁 route_revision | 三頁截圖對照（需登入後操作） | ⚠️ |
...
| AC-12 | Viewer 用邀請連結加入 | IT：Viewer／Editor 直接呼叫 API 越權被拒；SQL `permissions` 34 項、`sharing`；邀請預覽頁不含行程內容 | 真機點邀請連結 | ⚠️ |
...
- AI：`parse-import`、`ask-trip` 記錄延遲、token 數、狀態（不記 prompt，D11）；App 端也記錄 `ai.ask` 延遲。
