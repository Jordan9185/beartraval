...
- **高（H1–H6）**：全部已修正並加測試（commit `6b04396`）。H2 的做法改為「空白的日子只能排為當天第一站、路程顯示無法估算」，不直接拒絕，避免空白日子完全不能安排。
- **中（後端與 AI）**：全部已修正（`6b04396`）。共用地點的處理方式：陌生人不能補其他旅程在用的地點名稱；同一個地圖 ID 位置差超過 1 公里時各自存一筆。仍有一個殘留風險：某個地圖 ID 第一次被登錄時的名稱由先登錄的人決定。
- **中（畫面與流程）**：全部已修正。
- **視覺風格**：主色採 **A. 熊毛棕**（D13 已決定）。`findings.json` 的 100 項已逐條處理：第一批 15 項之外，另完成地圖圖層改成單一選單、每天的路線／交通方式／時區合成一列、路段不重複交通方式、購物清單依狀態分組並顯示「第 N 天」、搜尋欄與候選列共用元件、收藏換旅程改到工具列、成員頁移除重複的顯示名稱、App 圖示改成單色底只留熊熊等。
- **低嚴重度（程式 18 項、UX 10 項）**：已處理（2026-09-25 第二輪）。
- **未驗證的程式問題 46 項**：已逐條驗證；成立的已修正，其中 8 項在第一輪已修。
- **未驗證的 UX 建議 233 項**：`findings.json` 只保留數量，沒有內容，無法處理；需要時請重跑 UX 審查。
