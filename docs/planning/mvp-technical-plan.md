...
2. **插入位置**：k = 0..n+1。中間位置 `Δk = t(Sk, C) + t(C, Sk+1) − t(Sk, Sk+1)`；頭尾位置為 `t(C, S0)` 或 `t(Sn, C)`（頭尾是否允許插入【決策 D9】：若當日以飯店為起訖，只允許中間；D9 尚未實作，目前一律允許頭尾插入，見 §6）。
...
- **D12（已決策，2026-09-24）**：MVP 不提供手動輸入分鐘數；視實地測試回饋再議。
...
**之後補充**：D12（2026-09-24）、D13（2026-09-25）已決策。D9 雖已決策但**尚未實作**，改列【需實測或決策】：實作前要再確認是否仍需要。
...
| D9 | 每日起訖點 | 無 / 每日可設住宿為起訖 | Route Match 頭尾插入是否合理 | 【需實測或決策】原決定「**可設住宿**；未設時允許頭尾插入」，**尚未實作**：Route Match 已有 `allowEndpoints` 參數，但資料模型與 UI 沒有「住宿為起訖」設定，目前每天都允許頭尾插入 |
...
| D12 | 外開在地地圖後可否手動輸入分鐘數 | 不做 / 允許並標記「使用者輸入」 | 首爾大眾運輸日可顯示數字，但資料可信度不同 | **MVP 不做**（2026-09-24 已決策），視實地測試回饋再議（§4.3.1） |
| D13 | 品牌主色 | A. 熊毛棕（Logo 的熊與字樣）/ B. 行李箱藍 `#2E9CCA` | B 與 iOS 預設藍接近，品牌感弱 | **A. 熊毛棕 `#9A6440`（深色模式 `#C8925F`）**（2026-09-25 已決策）；只用在可點的元素，見 `docs/review/style-guide.md` |
...
1. ~~回覆第 6 節 D1–D11~~ 已完成（2026-09-24，全部採建議）；D12、D13 已補決策；D9 尚未實作，待確認。
2. ~~S1–S4 與 WP1–WP11~~ 已完成（見 [MVP 驗收報告](../acceptance/mvp-acceptance.md)）。接下來是真機、雲端與首爾／廣島實地驗收，以及 2026-09-25 review（`docs/review/`）的後續修正。
